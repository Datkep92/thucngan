...
function openMatchByDragPopup(taxCode) {
  ...
  if (!stockHtml) stockHtml = '<p style="text-align:center; color:#999; padding:20px;">Không còn tồn kho!</p>';
    ...
    <div style="padding:8px 10px; background:#fdfdfd; border-bottom:1px solid #eee; display:flex; flex-wrap:wrap; align-items:center; gap:6px;">
      <span style="color:#444;">💰 Lọc theo giá trị:</span>
      <input type="number" id="priceValueInput" 
             placeholder="Nhập giá trị (vd: 120000)" 
...
function renderFilteredLists() {
  ...
  if (activeFilters.unit) {
    const exportUnits = new Set(exportFiltered.map(i => i.unit));
    stockFiltered = stockFiltered.filter(s => exportUnits.has(s.unit));
  }

// --- LỌC GIÁ (SO SÁNH 2 CHIỀU) ---
if (activeFilters.priceDiff) {
  let threshold = 0.1;
  if (activeFilters.priceDiff === 'lt10') threshold = 0.1;
  if (activeFilters.priceDiff === 'lt15') threshold = 0.15;
  if (activeFilters.priceDiff === 'eq0') threshold = 0; // bằng tuyệt đối
  ...
  for (const exp of exportFiltered) {
    for (const stock of stockFiltered) {
      const p1 = parseFloat(exp.price) || 0;
      const p2 = parseFloat(stock.price) || 0;
      if (!p1 || !p2) continue;

      const diffRatio = Math.abs(p1 - p2) / ((p1 + p2) / 2);

      if (
        (threshold === 0 && Math.abs(p1 - p2) < 1) || // =0% (coi như bằng)
        (threshold > 0 && diffRatio <= threshold)
      ) {
  ...
  // --- LỌC THEO GIÁ NHẬP TAY (±15%) ---
  if (activeFilters.priceValue) {
    const target = activeFilters.priceValue;
    const min = target * 0.85;
    const max = target * 1.15;

    exportFiltered = exportFiltered.filter(e => {
      const p = parseFloat(e.price) || 0;
      return p >= min && p <= max;
    });
    stockFiltered = stockFiltered.filter(s => {
      const p = parseFloat(s.price) || 0;
      return p >= min && p <= max;
    });
  }

  // --- LỌC THEO NÚT NHANH (priceTarget) : ±15% quanh target ---
  ...
  // --- LỌC GIÁ THEO SO SÁNH CẶP (lt10 / eq0 / lt15) ---
  if (activeFilters.priceDiff) {
    let threshold = 0.1;
    if (activeFilters.priceDiff === 'lt10') threshold = 0.1;
    if (activeFilters.priceDiff === 'lt15') threshold = 0.15;
    ...
    for (const exp of exportFiltered) {
      for (const stock of stockFiltered) {
        ...
        if (
          (threshold === 0 && Math.abs(p1 - p2) < 1) || // coi như bằng
          (threshold > 0 && diffRatio <= threshold)
        ) {
          matchedExport.add(exp);
          matchedStock.add(stock);
  ...
  // --- LỌC TRÙNG KÝ TỰ (CỤM LIÊN TỤC) + TÔ MÀU ---
  ...
  function colorize(html, name) {
    ...
    return html.replace(
      /<div class="match-item"/,
      `<div class="match-item" style="background:${color}"`
    );
  }

  // --- RENDER ---
  document.getElementById('exportItemsList').innerHTML =
    exportFiltered.map(i => colorize(i.html, i.name)).join('') ||
    '<p style="text-align:center;color:#999;">Không có dữ liệu</p>';

  document.getElementById('stockItemsList').innerHTML =
    stockFiltered.map(i => colorize(i.html, i.name)).join('') ||
    '<p style="text-align:center;color:#999;">Không có dữ liệu</p>';

  console.log('🎯 Applied filters:', activeFilters, { exportFiltered, stockFiltered });
...
function autoMatchAndDeduct(taxCode, exportRecord) {
  ...
  hkd.tonkhoMain.forEach((stock, idx) => {
const key = stock.name.trim(); // Giữ nguyên ký tự gốc, có phân biệt hoa thường
    stockMap.set(key, { 
      ...stock, 
      originalIndex: idx,
